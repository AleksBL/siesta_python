...
class SiP:
    ...
    def calculate_bulksurf_RSSE(self, Ei, tx, ty, init_grid=None, 
        ...
        if init_grid is None:
            init_grid = grid_init(4)
            init_grid = init_grid[init_grid[:,1]<0.5+1e-3]
        tri= Delaunay(init_grid)
        pnts, simpl = tri.points, tri.simplices
        Hs = sisl.get_sile(self.dir+'/'+self.sl+'.TSHS').read_hamiltonian()
        He = sisl.get_sile(self.elecs[0].dir+'/'+self.elecs[0].sl+'.TSHS').read_hamiltonian()
        
        eorb = np.hstack([Hs.a2o(i) for i in self.elec_inds[0]])
        if self.elec_bloch is None:
            se_tx = 1
            se_ty = 1
        else:
            se_tx = self.elec_bloch[0]
            se_ty = self.elec_bloch[1]
        calc = surface_self_energy(Hs, He, tx, ty, eorb, 
                                   se_tx=se_tx, se_ty=se_ty,)
        Htot = Hs.tile(tx,0).tile(ty,1)
        Htot.set_nsc((1,1,1))
        H0 = Htot.Hk();      S0 = Htot.Sk()
        iH,jH,vH = find(H0); iS,jS,vS = find(S0)
        SEs = []; Idx = []; inv = np.linalg.inv
        for i,ei in enumerate(Ei):
            t1    = time()
            gi, err, nsd, feval = do_integrate(calc.gf, pnts, simpl, ei, tol, minarea)
            t2    = time()
            print('Integration at '+str(np.round(ei, 5))+' took: '+ str(np.round(t2-t1,3))+' sec')
            gi += gi.T; gi = inv(gi)
            gi[iH, jH] += vH
            gi         *= -1.0
            gi[iS, jS] += ei*vS
            #idx = np.where(np.abs(gi)>(tol/10))
            idx = np.where(np.abs(gi)>1e-10)
            SEs+= [gi[idx]]; Idx += [idx]
        Xidx = np.array([],dtype=np.int32)
        for idx,jdx in Idx:
            Xidx = np.union1d(np.unique(idx), Xidx)
        Xidx = np.unique(Htot.o2a(Xidx))
        Xio  = np.stack([Htot.a2o(i) for i in Xidx])
        SE_v = np.zeros((len(Ei), len(Xio), len(Xio)),dtype=np.complex64)
        for i in range(len(Ei)):
            gi[:,:]     = 0.0
            gi[Idx[i]]  = SEs[i]
            SE_v[i,:,:] = gi[Xio][:, Xio]
        H0 = H0[Xio[:, None], Xio[None, :]].toarray()
        S0 = S0[Xio[:, None], Xio[None, :]].toarray()
        xyz_elec = Htot.xyz[Xidx]
        return SE_v, Xio, H0, S0, xyz_elec
